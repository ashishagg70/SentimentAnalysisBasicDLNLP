...
class NeuralNet:
    ...
    def build_nn(self):
        ...
        self.model.add(tf.keras.layers.Dense(self.numOfOutputClasses,input_shape=(featureDim,)))
        #model.summary()
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(epochs):
                ...
                for i in range(0, self.reviews.shape[0], batch_size):
                    _, minibatch_cost = sess.run([optimizer, cost], feed_dict={X: shuffled_train_input[i:i + batch_size], Y: shuffled_train_target[i:i + batch_size]})
    ...
    def predict(self, reviews):
        ...
        '''reviewsTensor=tf.convert_to_tensor(reviews)
        predictions = self.model(reviewsTensor, training=False)
        return predictions'''
